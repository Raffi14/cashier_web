...
export async function POST(req: NextRequest) {
  try {
    const user = await Auth(req);
      ...
      await db.insert(users).values({ username, full_name, role, password: hash, is_active: "active" }).execute();
...
export async function GET(req: NextRequest) {
  try {
    ...
    const getUsers = await db.select().from(users).where(eq(users.is_active, "active")).orderBy(desc(users.id)).execute();
